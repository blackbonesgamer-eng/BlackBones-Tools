...
function ConvertirSeleccionMultiple($entrada, $maximo) {
    ...
    foreach ($parte in $partes) {
        ...
        if ($valor -match '^(\d+)-(\d+)$') {
            ...
            for ($n = $inicio; $n -le $fin; $n++) {
                $idx = $n - 1
                if ($idx -lt 0 -or $idx -ge $maximo) { return $null }
                if (-not $resultado.Contains($idx)) { $resultado.Add($idx) }
            }
        }
    ...
    while ($true) {
        ...
        switch ($key.VirtualKeyCode) {
            13 {
                ...
                Write-Host ""
                $sel = Read-Host "Elige número(s) de la lista visible (ej: 1 3 5 o 1,3,5 o 2-4) o B para volver"

                if ($sel -match '^[Bb]$') { continue }

                $indices = @(ConvertirSeleccionMultiple $sel $visibles.Count)

                if ($null -ne $indices -and $indices.Count -gt 0) {
                    $seleccionados = foreach ($idx in $indices) { $visibles[$idx] }
