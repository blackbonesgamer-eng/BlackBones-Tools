...
function ObtenerNombreToken($archivo) {
...
function Buscar-Tokens {
    ...
    while ($true) {
        ...
        if ([string]::IsNullOrWhiteSpace($busqueda)) {
            ...
            $resultados = @($Tokens | Where-Object {
                $_.Nombre -like "*$busqueda*" -or $_.Archivo -like "*$busqueda*"
            })
        }

        if (-not $resultados -or $resultados.Count -eq 0) {
            Write-Host "No se encontraron resultados para: $busqueda" -ForegroundColor Yellow
            continue
        }
        ...
        for ($i = 0; $i -lt $resultados.Count; $i++) {
            Write-Host ("[{0}] {1}  ->  {2}" -f ($i + 1), $resultados[$i].Nombre, $resultados[$i].Archivo) -ForegroundColor Yellow
        }

        Write-Host ""
        $seleccion = Read-Host "Elige un numero para instalar, escribe varios separados por coma, o B para volver a buscar"

        if ($seleccion -match '^[Bb]$') {
            continue
        }
        ...
        $indicesValidos = @()
        $selecciones = $seleccion -split ','

        foreach ($item in $selecciones) {
            $valor = $item.Trim()
            ...
            if ($indice -lt 1 -or $indice -gt $resultados.Count) {
                $indicesValidos = @()
                break
            }

            $indicesValidos += ($indice - 1)
        }

        if ($indicesValidos.Count -gt 0) {
            $tokensElegidos = foreach ($indice in ($indicesValidos | Select-Object -Unique)) {
                $resultados[$indice]
            }

            return @($tokensElegidos)
        }
...
function ActivarJuegos {
    ...
    $tokens = @(
        ...
        ForEach-Object {
            [PSCustomObject]@{
                Nombre = ObtenerNombreToken $_.name
                Archivo = $_.name
                Url = $_.download_url
            }
        } |
        Sort-Object Nombre, Archivo
    )

    $tokensElegidos = Buscar-Tokens -Tokens $tokens

    if ($null -eq $tokensElegidos -or $tokensElegidos.Count -eq 0) {
    ...
    Write-Host ""
    Write-Host "Instalando tokens seleccionados..." -ForegroundColor Cyan

    foreach ($token in $tokensElegidos) {
        $destFile = Join-Path $Destino $token.Archivo
        Write-Host "➡ $($token.Archivo)" -ForegroundColor DarkYellow
        Invoke-WebRequest $token.Url -OutFile $destFile -UseBasicParsing
    }
